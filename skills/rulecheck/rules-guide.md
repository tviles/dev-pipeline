...
| Rule | Violation |
|------|-----------|
| No generic `import *` from project modules | `import * as auth from './auth'` |
| Use `import type` for type-only | `import { MyType } from '...'` when MyType is only used as a type |
| Namespace imports for deep paths only | `import * as users from './services/users/db'` is fine |
| Specific named imports for values | `import { verifyToken, signToken } from './auth'` |
...
| Use the project's git wrapper functions | Direct git shell-out calls when a wrapper utility exists |
...
- **Generic `import *`** — from project modules instead of specific named imports
