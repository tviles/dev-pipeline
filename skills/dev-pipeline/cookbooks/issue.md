...
- {For bugs}: `/dev-pipeline debug #{number}` to investigate
- {For features}: `/dev-pipeline prd` to write requirements, or `/dev-pipeline plan` to design
```
...
```
/dev-pipeline issue                              # Use recent conversation context
/dev-pipeline report this to gh                  # Same — infer from context
/dev-pipeline log this bug in github             # Bug from context
/dev-pipeline create a feature request for X     # Feature with description
/dev-pipeline gh issue: streaming breaks on large responses  # Bug with description
```
