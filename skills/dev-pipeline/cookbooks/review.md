...
Check `.claude/dev-pipeline/` for artifacts related to this work:

```bash
ls .claude/dev-pipeline/reports/ 2>/dev/null
ls .claude/dev-pipeline/plans/completed/ 2>/dev/null
ls .claude/dev-pipeline/issues/ 2>/dev/null
ls .claude/dev-pipeline/debug/ 2>/dev/null
```
...
Save to `.claude/dev-pipeline/reviews/{date}-pr-{number}.md` (or `{date}-{slug}.md` for local changes).
...
gh pr review {NUMBER} --approve --body-file .claude/dev-pipeline/reviews/{filename}.md

# If --request-changes OR high issues:
gh pr review {NUMBER} --request-changes --body-file .claude/dev-pipeline/reviews/{filename}.md

# Otherwise just comment:
gh pr comment {NUMBER} --body-file .claude/dev-pipeline/reviews/{filename}.md
```
...
Suggest: `/dev-pipeline review {pr-number}` to self-review before requesting human review.
