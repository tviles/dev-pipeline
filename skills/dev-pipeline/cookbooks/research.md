...
If `--follow-up` and an existing research file on this topic exists in `.claude/dev-pipeline/research/`:
...
Save to `.claude/dev-pipeline/research/{date}-{slug}.md`.
...
**Next steps**:
- To dig deeper: `/dev-pipeline research --follow-up {topic}`
- For strategic/external research: `/dev-pipeline investigate {topic}`
- To write requirements: `/dev-pipeline prd {topic}`
