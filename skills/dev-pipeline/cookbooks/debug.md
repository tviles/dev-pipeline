...
> **Authoritative debugging protocol.** Do NOT invoke `superpowers:systematic-debugging` — its four-phase protocol (investigate → isolate → fix → verify) is already covered by Phases 1–5 below, with stricter evidence standards. Running both is redundant and produces duplicate hypothesis tables.
...
**If from a GitHub issue**: Save to `.claude/dev-pipeline/issues/issue-{number}.md`
**Otherwise**: Save to `.claude/dev-pipeline/debug/{date}-{slug}.md`
...
**Next steps**:
- For complex fixes: `/dev-pipeline plan` (create a plan from the RCA)
- For simple fixes: `/dev-pipeline implement` (implement directly)
