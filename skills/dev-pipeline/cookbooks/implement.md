...
> **Authoritative implementation protocol.** Do NOT invoke any of the following superpowers skills while running this cookbook — they duplicate phases already covered here and produce conflicting workflows:
> - `superpowers:executing-plans` — Phases 1–3 (LOAD / PREFLIGHT / EXECUTE) cover this
> - `superpowers:subagent-driven-development` — this cookbook executes sequentially by design; do not switch to subagent dispatch mid-run
> - `superpowers:verification-before-completion` — Phase 4 (VALIDATE) is the multi-level validation gate
>
> TDD (`superpowers:test-driven-development`) is allowed *only* if the plan file explicitly calls for it; otherwise follow the plan's task order.
...
2. **If no path**: Look for the most recent `.plan.md` in `.claude/dev-pipeline/plans/` (excluding `completed/`)
3. **If no plans found**: Tell the user and suggest `/dev-pipeline plan`
...
Save to `.claude/dev-pipeline/reports/{slug}-report.md` using the plan's slug.
...
```bash
mkdir -p .claude/dev-pipeline/plans/completed
mv {plan-path} .claude/dev-pipeline/plans/completed/
```
...
To continue: `/dev-pipeline plan {prd-path}`
```

**Next steps**:
- To commit: `/dev-pipeline commit`
- To create PR: `/dev-pipeline pr`
- To review: `/dev-pipeline review`
