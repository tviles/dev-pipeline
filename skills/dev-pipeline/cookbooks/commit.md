...
- Files committed
- Suggest next step: `/dev-pipeline pr` to create a pull request

### Examples

```
/dev-pipeline commit                          # All changes
/dev-pipeline commit typescript files         # *.ts only
/dev-pipeline commit except package-lock      # Exclude specific
/dev-pipeline commit only the new files       # Untracked only
/dev-pipeline commit staged                   # Already-staged only
/dev-pipeline commit the auth refactor        # Context-based targeting
```
